...
const useComponentState = (props, { animationDriver }, staticConfig, config) => {
  const isHydrated = (0, import_use_did_finish_ssr.useDidFinishSSR)(), [startedUnhydrated] = (0, import_react.useState)(import_constants.IS_REACT_19 ? !isHydrated : !1), useAnimations = animationDriver?.useAnimations, stateRef = (0, import_react.useRef)(
    void 0
  );
  ...
  import_constants.isWeb && hasAnimationThatNeedsHydrate && !staticConfig.isHOC && !isHydrated && (isAnimated = !1, curStateRef.willHydrate = !0), disabled !== state.disabled && (disabled && Object.assign(state, import_defaultComponentState.defaultComponentStateMounted), state.disabled = disabled, setState({ ...state }));
