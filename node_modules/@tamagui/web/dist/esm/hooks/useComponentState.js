...
const useComponentState = (props, { animationDriver }, staticConfig, config) => {
  const isHydrated = useDidFinishSSR(), [startedUnhydrated] = useState(IS_REACT_19 ? !isHydrated : !1), useAnimations = animationDriver?.useAnimations, stateRef = useRef(
    void 0
  );
  ...
  isWeb && hasAnimationThatNeedsHydrate && !staticConfig.isHOC && !isHydrated && (isAnimated = !1, curStateRef.willHydrate = !0), disabled !== state.disabled && (disabled && Object.assign(state, defaultComponentStateMounted), state.disabled = disabled, setState({ ...state }));
